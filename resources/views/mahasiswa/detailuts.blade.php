...
<div class="row mt-4">
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="card">
            <h5 class="card-header text-center">Uts</h5>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered first">
                        ...
                        <tbody>
                           ...
                           @foreach ($all_nilai_uts as $item)
                           <tr id="items">
                               <td class="text-center">{{ $loop->iteration}}</td>
                               <td>{{ $item->mahasiswa->nama }}</td>
                               <td><a href="" class="editableuts" data-type="text" data-name="uts" data-pk="{{ $item->id }}">{{ $item->uts }}</td>
                               <td class="text-center">
                               <form action="/nilaiuts/hapusuts/{{ $item->id }}" method="post" class="d-inline">
                                   @method('DELETE')
                                   @csrf
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <form id="form_create_nilai_uts">
                <input type="hidden" name="mahasiswa_id" id="mahasiswa_id" value="{{ $id }}">
                <input type="number"  class="form-control  @error('uts') is-invalid @enderror" value="{{old('uts')}}" name="uts" id="uts" >
